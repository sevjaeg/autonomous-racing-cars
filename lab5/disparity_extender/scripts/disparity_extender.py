...
if USE_DYNAMIC_RECONFIG:
    from disparity_extender.cfg import GainsConfig
...
MAX_SPEED = rospy.get_param('/disparity_extender/max_speed', 6.3)  # m/s  (only without basic velocity)
MIN_SPEED = rospy.get_param('/disparity_extender/min_speed', 1.7)  # m/s  (only without basic velocity)
LOOKAHEAD_DIST_FAST = 3.0  # m  (if the car drives more than 5 m/s)
LOOKAHEAD_DIST_MID = 2.25  # m  (if the car drives more than 3 m/s)
LOOKAHEAD_DIST_SLOW = rospy.get_param('/disparity_extender/lookahead_dist_slow', 1.5) # m  (if the car drives slower than 3 m/s)
